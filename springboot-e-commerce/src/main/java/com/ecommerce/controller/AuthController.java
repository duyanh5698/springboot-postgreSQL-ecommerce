...
public class AuthController {
	...
	public ResponseEntity<?> authenticateUser(@Valid @RequestBody LoginRequest loginRequest) {
		...
		if (user == null)
			throw new RestClientException("not found user");
	...
	@PostMapping("/signup")
	public ResponseEntity<?> registerUser(@Valid @RequestBody SignupRequest signUpRequest) throws RestApiException {
		...
		User user = new User(signUpRequest.getUsername(), signUpRequest.getEmail(), signUpRequest.getAge(),
				encoder.encode(signUpRequest.getPassword()));

		Set<Role> roles = new HashSet<>();

		Role userRole = roleRepository.findByName(ERole.ROLE_USER)
				.orElseThrow(() -> new RestApiException("Error: There is no 'ROLE_USER' in the database."));
		roles.add(userRole);
